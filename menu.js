...
(function(Menu) {
  ...
  var menus,tfid='menutf',mok,mcancel,lsx=0,lsy=0,touchms=[],keym={},gpbum={},gppress=new Array(16),
      nomouse=false,ccw=1,cch=1,pressed=[],pressids={},ps,mD=false;
  ...
  Menu.draw=function() {
    ...
    var cy0=Math.floor(Menu.py*ccw+0.5),cy=cy0;
    ...
    //      var cos=m.mctrl.cos;
      ...
      //if (m.py===undefined) m.py=0.02+(m.ph)*i;
        ...
        m.cw=Math.floor(m.pw*ccw+0.5)-(cos?0:padding*2);
        m.ch=Math.floor(m.ph*(m.yvh?cch:ccw)+0.5)-(cos?0:padding*2);
        m.cy=Math.floor(m.py*(m.yvh?cch:ccw)+0.5);if (m.ydown) m.cy=cch-m.cy-m.ch;
      } else {
        m.cx=cx-(m.pw?Math.floor(m.pw*ccw+0.5)-cw:0);
        m.cy=cy;//+i*ch;
        m.cw=m.pw?Math.floor(m.pw*ccw+0.5):cw;
        m.ch=ch-1;
        cy+=ch;
        if (cy+ch+ch>cch) {
          cx-=cw+1;
      ...
      if (m.tf) {
        ...
        +';position:absolute;'+(m.c2?'z-index:10;text-shadow:1px 0px 1px '+(m.tbgcol?m.tbgcol:'#aaa')+';':'')+'top:'+(ch-2*mfs)+'pt;left:2pt;">'+m.ms+'</div>':'');
  ...
  Menu.keyDown=function(ev) {
    ...
    if (m) { if (ev.ctrlKey) ev.preventDefault();m.on=true;if (m.c) m.c.style.backgroundColor=Menu.colOver;if (m.actionf) { m.actionf();ret=1; }}
    return ret;
    //alert(kc);
  }
  ...
  */
  ...
  function mouseMove(e) {
    if (!mD||Menu.press) Menu.search(e.pageX,e.pageY);
  }
  function mouseUp(e) {
    mD=false;
    Menu.mouseUp();
