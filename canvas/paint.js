...
(function(Paint) {
  ...
  Paint.loaded=function(aco,acoH_) {
    ...
    {s:'Load',ms:'localStorage',actionf:lsLoad},
    {s:'Save',ms:'localStorage',actionf:function() {
      //...
      try {
      ...
      var d=serialize(1);
      localStorage[lsKey+'json']=d;
      
      log('Saved to localStorage ('+d.length+' bytes).');
      } catch (e) {
        log(''+e);
      }
      //lert('save');
    ...
    ,{s:'Process cut',ms:'at current index',actionf:function() {
      //...
      if (pri<pra.length-1) 
        pra.splice(pri+1,pra.length-1-pri);
      log('Process cut done.');
    }
    }
    ...
    ,{s:'Paint '+version+'<br>Menu '+Menu.version+'<br>Conet '+Conet.version,fs:0.6,noinp:1,_vertCenter:1}
